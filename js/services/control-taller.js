...
function areaDeUnidad(u){const combustible=normalizarTaller(u.combustible),grupo=normalizarTaller(u.grupo),texto=normalizarTaller([u.descripcion,u.unidad_patrulla,u.modelo].join(" "));if(combustible==="electrico"||combustible==="hibrido")return"Electricas";if(/colector/.test(texto))return"Colectores";if(/barredora/.test(texto))return"Barredoras";if(/motocicleta|moto\b/.test(texto))return"Motocicletas";if(grupo.includes("maquinaria pesada"))return"Maquinaria Pesada";if(combustible==="gasolina")return"Gasolina";if(combustible==="diesel")return"Diesel";return"Sin área"}
...
function diasEnTaller(i){const fin=i.fecha_salida?new Date(i.fecha_salida+"T00:00:00"):new Date();const ini=new Date(i.fecha_ingreso+"T00:00:00");return Math.max(0,Math.floor((fin-ini)/86400000))}
...
async function cargarControlTaller(){if(!tallerDb?.from)return;configurarIdentidadArea();const [ingresos,unidades,pendientes]=await Promise.all([cargarTodosTaller("ingresos_taller","*",{columna:"fecha_ingreso",ascendente:false}),cargarTodosTaller("parque_vehicular","id,numero_economico,numero_inventario,unidad_patrulla,descripcion,marca,modelo,grupo,combustible,dependencia,disponibilidad",{columna:"id",ascendente:true}),cargarTodosTaller("ingresos_taller_pendientes","*",{columna:"creado_en",ascendente:true})]);if(ingresos.error||unidades.error){console.error(ingresos.error||unidades.error);alert("No se pudo cargar Control de Área. Verifica que las migraciones estén aplicadas.");return}if(pendientes.error)console.warn("Pendientes de taller no disponibles:",pendientes.error.message);const todasUnidades=unidades.data||[];unidadesTaller=accesoGlobalControl?todasUnidades:todasUnidades.filter(u=>areasControl.includes(areaDeUnidad(u)));const ids=new Set(unidadesTaller.map(u=>u.id));ingresosTaller=(ingresos.data||[]).filter(i=>accesoGlobalControl||ids.has(i.vehiculo_id)||areasControl.includes(i.area));pendientesTaller=((pendientes.error?[]:pendientes.data)||[]).filter(i=>accesoGlobalControl||areasControl.includes(i.area));poblarProveedoresTaller();actualizarDetalleUnidadTaller();renderizarTaller();renderizarPendientesTaller()}
...
function resumenDependencias(){const abiertas=ingresosTaller.filter(i=>i.estatus!=="Terminado"),map=new Map;unidadesTaller.forEach(u=>{const d=u.dependencia||"Sin dependencia";if(!map.has(d))map.set(d,{dependencia:d,total:0,taller:0});map.get(d).total++});abiertas.forEach(i=>{const d=i.dependencia||"Sin dependencia";if(!map.has(d))map.set(d,{dependencia:d,total:0,taller:0});map.get(d).taller++});return Array.from(map.values()).map(r=>Object.assign(r,{activas:Math.max(0,r.total-r.taller)})).sort((a,b)=>b.total-a.total)}
